...
func (e *APIResponseError) StatusCode() int { return e.cause.StatusCode }

// Errors returns the individual error entries in the response.
func (e *APIResponseError) Errors() []*Error {
	out := make([]*Error, len(e.cause.Errors))
	for i, ce := range e.cause.Errors {
		out[i] = &Error{
			Message:  ce.Message,
			Code:     ce.Code,
...
type InternalClientError struct {
	cause *client.InternalClientError
}

// Error implements the error interface.
func (e *InternalClientError) Error() string { return e.cause.Error() }
...
func convertError(err error) error {
	...
	case *client.APIResponseError:
		return &APIResponseError{cause: e}
	...
	case *client.InternalClientError:
		return &InternalClientError{cause: e}
	default:
		return err
	}
}
