...
func Test_contextPropagation(t *testing.T) {
	...
	cases := []struct {
		...
		{"Service.Me", func(t *testing.T, m *client.Method) {
			m.Get = makeMockFn(t)
			s := user.NewService(m)
			s.Me(ctx) //nolint:errcheck
		}},
		{"Service.Add", func(t *testing.T, m *client.Method) {
			m.Post = makeMockFn(t)
			s := user.NewService(m)
		...
		{"StarService.List", func(t *testing.T, m *client.Method) {
			m.Get = makeMockFn(t)
			s := user.NewStarService(m)
			s.List(ctx, 1) //nolint:errcheck
		}},
		{"StarService.Count", func(t *testing.T, m *client.Method) {
			m.Get = makeMockFn(t)
			s := user.NewStarService(m)
